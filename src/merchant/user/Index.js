...
const Index = () => {
  ...
  const getAllMerchants = async () => {
    setLoading(true);
    const response = await MerchantService.getAllMerchants(lazyParams);
  ...
  const deleteMerchant = async (id) => {
    if (await MerchantService.deleteMerchant(id)) {
      getAllMerchants();
      toast.current.show({
        severity: "success",
  ...
  return (
    <Fragment>
      <Breadcrumb items={[{ label: "Merchants" }]} />
          ...
          <div className="card">
            {items && (
              <>
                <Toolbar
                  className="mb-4"
                  left={<LeftToolbarTemplate name={"merchant"} />}
                ></Toolbar>
                <DataTable
                  value={items}
                  lazy
                  ...
                  <Column
                    body={(data) => (
                      <TableAction
                        data={data}
                        name={"merchant"}
                        deleteItem={(id) => deleteMerchant(id)}
                      />
                    )}
                    exportable={false}
                  />
